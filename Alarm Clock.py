...
class CreateAlarm:
    def __init__(self, master):
        ...
        Label(self.master, text='Time', bg='white',
              fg='red').pack(anchor='center')
        Label(self.master, textvariable=self.time, font=self.font,
              bg='white', fg='red').pack(anchor='center')

        Label(self.master, text='Date', bg='white',
              fg='red').pack(anchor='center')
        Label(self.master, textvariable=self.date, font=self.font,
              bg='white', fg='red').pack(anchor='center')

        Button(self.master, fg=self.foreground, text='Set Alarm', command=self.popup).pack()
    ...
    def popup(self):
        ...
        Hour = Entry(HF, width=8, justify='center', bg=self.background)
        ...
        Minute = Entry(MF, width=8, justify='center', bg=self.background)
        Minute.pack(pady=5, padx=5)

        SF = LabelFrame(LF, text='Second', bg=self.background, fg=self.foreground, bd=0)
        SF.grid(row=0, column=3)

        Second = Entry(SF, width=8, justify='center', bg=self.background)
        Second.grid(pady=5, padx=5)
        
        Button(LF, text='Set Alarm', fg=self.foreground, command=lambda: self.setAlarm(Hour, Minute, Second, win)).grid(row=1, column=2)
